...
- 표준 입력을 받아들여 표준 출력으로 출력하는 명령어
- ex) cat

### head: 대표적인 필터 명령어

- 지정한 파일의 첫행부터 지정한 행까지 출력
- 기본값은 첫 10행을 출력하는 것
...
- 입력 파일을 지정하지 않으면 표준 입력을 입력으로 사용.
...
```bash
$ du -b /bin/* <- /bin 디렉터리 아래의 모든 파일의 크기를 바이트 단위로 출력
$ du -b /bin/* | sort -n <- du 명령어의 출력 결과를 sort 명령어로 크기가 작은 순으로(-n) 정렬
$ du -b /bin/* | sort -n | tac <- 출력 결과를 크기가 큰 순으로 정렬
$ du -b /bin/* | sort -n | tac | head -n 5 <-크기가 가장 큰 상위 5개만 출력
```